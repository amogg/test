...
def pjl_download_file(file_path, save_as):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            s.settimeout(20) # 파일 크기에 따라 타임아웃을 넉넉하게 설정
            s.connect((PRINTER_IP, PRINTER_PORT))
            ...
            while len(received_data) < file_size:
                # 남은 데이터 크기와 4096 중 더 작은 값만큼 수신 시도
                bytes_to_read = min(file_size - len(received_data), 4096)
            ...
            if len(received_data) == file_size:
                ...
                print(f"\n[+] 성공: 파일 '{save_as}' ({file_size} bytes) 저장을 완료했습니다.")
                return True
            else:
                print(f"\n[!] 오류: 예상 크기({file_size} bytes)와 다른 크기({len(received_data)} bytes)의 데이터를 수신했습니다.")
                return False
